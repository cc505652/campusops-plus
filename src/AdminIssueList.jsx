...
export default function AdminIssueList() {
  ...
  const escalateIssue = async (issue) => {
    const ok = window.confirm(`Escalate issue to Warden?\n\n"${issue.title}"`);
    if (!ok) return;
    ...
    await updateDoc(doc(db, "issues", issue.id), {
      ...
      statusHistory: [
        ...(issue.statusHistory || []),
        { status: "escalated", at: Timestamp.now(), note: "Escalated to Warden" }
      ],
      updatedAt: serverTimestamp()
    });
  };
...
`.trim();
      ...
      try {
        ...
        aiNarration = "AI temporarily unavailable. Showing accurate report only.";
      }
    ...
    <div style={{ padding: 16 }}>
      ...
      <div style={{ border: "1px solid #ddd", borderRadius: 14, padding: 14, marginBottom: 16 }}>
        <div style={{ display: "flex", justifyContent: "space-between", gap: 12, flexWrap: "wrap" }}>
          <div>
            <b style={{ fontSize: 14 }}>Live Ops Highlights</b>
            <div style={{ fontSize: 12, opacity: 0.75 }}>What matters right now</div>
          </div>
      ...
      <div style={{ border: "1px solid #ddd", padding: 12, borderRadius: 12, marginBottom: 16 }}>
        <div style={{ display: "flex", gap: 12, alignItems: "center", flexWrap: "wrap" }}>
          ...
          <span style={{ fontSize: 12, opacity: 0.7 }}>Accurate stats + Gemini narration</span>
        </div>
      ...
      {galleryOpen && (
        ...
        >
          ...
          >
            <div style={{ display: "flex", justifyContent: "space-between", gap: 12, flexWrap: "wrap" }}>
              <div>
                <b style={{ fontSize: 16 }}>Evidence Gallery</b>
